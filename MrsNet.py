...
class MRSNet(nn.Module):
    def __init__(self, EncodingDepth=5):
        ...
        self.FinalConv = CConv1d(initial_num_layers, No_channels)
    ...
    def forward(self, x_r, x_i, k0_r, k0_i, mask):
        ...
        # x_i = x_i.permute(0, 2, 1).contiguous()
        x_r = x_r + INPUT_r  ##
        x_i = x_i + INPUT_i  ##
...
if __name__ == '__main__':
    ...
    x_r = torch.randn(2, 1, 256, dtype=torch.float)
    x_i = torch.randn(2, 1, 256, dtype=torch.float)
    print('input' + str(x_r.size()))
    print(x_r.dtype)
    ini_r, ini_i, y_r, y_i = mrsnet(x_r, x_i, x_r, x_i, x_r)
